 ...
 */
/*****************************************************************************
 * FILE NAME: c106392_mr_resync_customers_deploy_not_found.js
 * DEVOPS TASK: ENH 106392 (Optimization)
 *
 * DESCRIPTION:
 *  - When Customer UE tries to submit the main Customer->SF sync MR (address lines > threshold),
 *    some submissions can fail during high-volume CSV imports because no MR deployments are free.
 *  - The UE marks those Customers with a checkbox (Deployment Didn't Found).
 *  - This MR script picks up those flagged Customers and runs the SAME getCustomerFields()
 *    logic (copied from UE) to complete the sync, then clears the checkbox.
 *
 * NOTE:
 *  - Do NOT change the functional logic inside getCustomerFields(). It is intentionally heavy.
 *  - This MR runs in MAP_REDUCE context, so it has higher governance than UE.
 *****************************************************************************/
...
function (record, search, format, library, runtime) {
    ...
    var SF_ERROR_MESSAGE_FIELD = 'custentity_crw_100062_sfint_errormessage';
    ...
    function getInputData() {
        return search.create({
            ...
            filters: [
                [MR_DEPLOYMENT_NOT_FOUND_FIELD, 'is', 'T']
            ],
            columns: [
    ...
    function _clearDeploymentFlag(custId) {
        try {
            var lookup = search.lookupFields({
                type: search.Type.CUSTOMER,
                id: custId,
                columns: [SF_ERROR_MESSAGE_FIELD]
            });

            var errMsg = lookup && lookup[SF_ERROR_MESSAGE_FIELD];
            var hasErr = (errMsg && typeof errMsg === 'string' && errMsg.trim().length > 0);

            if (hasErr) {
                log.audit('Keeping MR deployment flag (SF error message present)',
                    'custId=' + custId + ', error=' + errMsg.substring(0, 300));
                return false;
            }

            var values = {};
            values[MR_DEPLOYMENT_NOT_FOUND_FIELD] = false;
            ...
            log.error('Unable to clear MR deployment flag',
                'custId=' + custId + ', error=' + (e && e.message ? e.message : e));
            return false;
        }
    }
    ...
    function map(context) {
        ...
        var custId = searchResult && searchResult.id;

        if (!custId) {
            log.error('Missing customer id in map input', context.value);
            return;
        }

        log.audit('Resync Customer (deployment flag)', 'custId=' + custId);

        try {
            var token = library.geturl();
            getCustomerFields(custId, token, runtime.executionContext);

            // _clearDeploymentFlag(custId);

            log.audit('Resync completed (flag cleared)', 'custId=' + custId);
        }
        catch (e) {
            log.error('Resync failed (flag kept for retry)',
                'custId=' + custId + ', error=' + (e && e.message ? e.message : e));
        }
    }
    ...
    function getCustomerFields(custId, token, runtimeexecutionContext) {
            try {
                ...
                log.debug('hasErrorForCategory', hasErrorForCategory);
